...
export const logoutHandler = async (req, res) => {
  ...
  try {
    ...
    if (!tokenToInvalidate) {
      return res
        .status(400)
        .json({ message: "Token not provided in the header" });
    }
    ...
    if (
      blacklist.has(userId) &&
      blacklist.get(userId).has(tokenToInvalidate)
    ) {
      return res.status(401).json({
        message: "Token already invalidated, session closed",
        originalToken: tokenToInvalidate,
        decodedToken,
      });
    }
    ...
    res.status(200).json({
      ...
      invalidatedToken: tokenToInvalidate,
      originalToken: tokenToInvalidate,
      decodedToken,
      userId,
    });
    ...
    if (error instanceof jwt.TokenExpiredError) {
      return res.status(401).json({
        message: "Token expired, session closed",
        originalToken: tokenToInvalidate,
      });
    }
